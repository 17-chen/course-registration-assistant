...
- 本地中文控制面板，关闭程序后停止运行
- 固定时间抢课：日期与时、分、秒滚轮设置
- 空位监控：后台持续刷新并读取 `Seats Available`
- 多个 Section 按输入顺序监控，任意一个成功后停止
- 演练模式：验证识别和通知，不执行最终注册
- 自动识别 `Register Now`、学校处理遮罩和注册结果
- 固定时间模式会提前预检 Planned 课程，到点时直接点击，不在关键时刻扫描页面
- 点击后识别 `Updating schedule...` 与 `Processing complete. Refreshing...`，处理完成前禁止刷新或重复提交
- 首次点击 2 秒后仍未进入处理状态时，只执行一次兜底点击；最终结果最长等待 60 秒
...
如果 Safari 在15秒内无法建立自动化会话，助手会停止等待并显示授权提示，不会一直卡在“正在打开”。
...
4. 设置刷新间隔并点击“开始运行”。
5. 助手持续刷新页面，直到手动停止或真实注册成功。

满员期间只刷新页面，不点击注册按钮。每次从满员变为有余位时最多触发一次提交；如果页面仍显示上一次的余位状态，助手会继续刷新确认，不会连续点击。只有重新观察到满员、之后又出现新余位时，才会再次触发提交。
